...
async function kg_get_playlist(listId) {
  ...
  const dom_data =  await new Promise(async (resolve, reject) => {
    jsdom.env(res_data.data, async function (errors, window) {
      const el = window.document.querySelectorAll('script');
      const elList = el._toArray();
      let globalData = [];
      elList.forEach(s => {
      ...
      const {specialInfo, data} = globalData;
      const info = {
      ...
      const tracks = await Promise.all(
        data.map(async item => await kg_render_playlist_result_item(item)),
      );
      resolve({
        info,
        tracks: tracks,
